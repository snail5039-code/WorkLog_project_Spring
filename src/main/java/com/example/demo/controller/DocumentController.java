...
public class DocumentController {
	...
	public DocumentController(DocumentGeneratorService documentGeneratorService, WorkLogService workLogService) {
	...
	public ResponseEntity<String> generateWorkLog(String title, String mainContent, String sideContent, String author,
		...
		try {
			...
			String baseOutputDir = "C:/temp/output/";
			String outputFilePath = baseOutputDir + outputFileName;
			// 혹여나 상위 디텍토리가 없으면 생성하라라는 것임
			Files.createDirectories(Paths.get(baseOutputDir));

			// 3. DocumentGeneratorService 호출하여 문서 생성
			byte[] docxBytes = documentGeneratorService.generateDocxReport(templateFile, templateData // 👈 준비된 실제 데이터를
																										// 전달합니다.
			);
			...
			if (files != null && !files.isEmpty()) {
				...
				System.out.println(
						"첨부 파일 " + (files.size() - 1) + "개 중 " + savedCount + "개가 " + attachmentDir + "에 저장되었습니다.");
			}
